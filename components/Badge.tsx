...
import { safeHex, parseHex } from "@/lib/color";
...
export function Badge({ children, size = "md", dot, icon, removable, onRemove, color = "#876cff", style }: BadgeProps) {
  ...
  const { r, g, b } = parseHex(validColor);
    ...
    <span style={{
      ...
      {dot && (
        <span style={{
          ...
          background: validColor, flexShrink: 0,
          boxShadow: `0 0 6px ${validColor}`,
        }} />
      )}
      ...
      {removable && (
        <button
          ...
          style={{
            ...
            color: validColor, cursor: "pointer", padding: 0, marginLeft: "2px",
